...
// Flash playback service: mobile writes "READ"/"RESET" to CMD, receives 243-byte chunks on DATA
#define FLASH_SERVICE_UUID      "19B30000-E8F2-537E-4F6C-D104768A1214"
#define FLASH_DATA_CHAR_UUID    "19B30001-E8F2-537E-4F6C-D104768A1214"  // NOTIFY 243 bytes
#define FLASH_CMD_CHAR_UUID     "19B30002-E8F2-537E-4F6C-D104768A1214"  // WRITE  "READ"/"RESET"
...
// Flash playback BLE service
BLEService* flashService = nullptr;
BLECharacteristic* flashDataCharacteristic = nullptr;  // notifies 243-byte chunks
BLECharacteristic* flashCmdCharacteristic  = nullptr;  // receives "READ" / "RESET"

// ===== FLASH-OVER-BLE PACKET CONSTANTS =====
// Each flash record is 9 bytes.  27 records x 9 bytes = 243 bytes per BLE packet.
#define FLASH_BLE_SAMPLES_PER_PKT  27
#define FLASH_BLE_PKT_SIZE         (FLASH_BLE_SAMPLES_PER_PKT * FLASH_RECORD_SIZE)  // 243

// Framing markers sent before the data stream and after it so the mobile app
// can reliably detect start / end even if it misses the first packet.
//   START marker: 0xAA 0xBB 0xCC 0xDD  (4-byte notify)
//   END   marker: 0xFF 0xEE 0xDD 0xCC  (4-byte notify)
static const uint8_t FLASH_BLE_START_MARKER[4] = {0xAA, 0xBB, 0xCC, 0xDD};
static const uint8_t FLASH_BLE_END_MARKER[4]   = {0xFF, 0xEE, 0xDD, 0xCC};

// Flags set by the BLE CMD callback, consumed in loop()
volatile bool flashReadRequested  = false;
volatile bool flashResetRequested = false;
...
// ===== FLASH CMD BLE CALLBACK =====
// Mobile writes:
//   "READ"  -> stream all stored flash data over FLASH_DATA_CHAR in 243-byte chunks
//   "RESET" -> erase all flash data and reset write pointer (clears storage)
class FlashCmdCharacteristicCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* pCharacteristic) {
    std::string value = pCharacteristic->getValue();
    if (value == "READ") {
      Serial.println("FLASH-BLE: READ command received");
      flashReadRequested = true;
    } else if (value == "RESET") {
      Serial.println("FLASH-BLE: RESET command received");
      flashResetRequested = true;
    } else {
...
// =====================================================================
// FLASH-OVER-BLE FUNCTIONS
// =====================================================================
//
// flashSendOverBLE()
//   Reads the entire stored flash dataset (from FLASH_HEADER_SIZE up to
//   flash_write_offset) and sends it to the connected mobile device in
//   243-byte packets (27 samples x 9 bytes each).
//
//   Packet structure on the wire:
//     [START_MARKER 4 bytes]           <- framing: 0xAA 0xBB 0xCC 0xDD
//     [TOTAL_BYTES  4 bytes, little-endian]  <- mobile knows how many bytes to expect
//     [data chunk 0  : 1..243 bytes]   <- 27 full records OR partial last chunk
//     [data chunk 1  : 1..243 bytes]
//     ...
//     [END_MARKER   4 bytes]           <- framing: 0xFF 0xEE 0xDD 0xCC
//
//   A 20ms delay is inserted between every notify to let the BLE stack
//   drain; increase if the mobile reports dropped packets.
//
void flashSendOverBLE() {
  if (!dataPartition) {
    Serial.println("FLASH-BLE: No partition — aborting send");
    flashReadRequested = false;
    return;
  }
  if (!wasConnected) {
    Serial.println("FLASH-BLE: Not connected — aborting send");
    flashReadRequested = false;
    return;
  }
  if (flash_bytes_written == 0) {
    Serial.println("FLASH-BLE: No data stored — sending END immediately");
    flashDataCharacteristic->setValue((uint8_t*)FLASH_BLE_START_MARKER, 4);
    flashDataCharacteristic->notify();
    delay(20);
    flashDataCharacteristic->setValue((uint8_t*)FLASH_BLE_END_MARKER, 4);
    flashDataCharacteristic->notify();
    flashReadRequested = false;
    return;
  }

  // Stop any ongoing flash recording before reading — avoids partial records
  if (flash_recording) {
    flashStopRecording();
    Serial.println("FLASH-BLE: Stopped recording before send");
  }

  uint32_t total_bytes = flash_bytes_written;
  Serial.printf("FLASH-BLE: Sending %lu bytes in %lu-byte packets\n",
                total_bytes, (uint32_t)FLASH_BLE_PKT_SIZE);

  // --- START marker + total byte count (4 bytes, little-endian) ---
  flashDataCharacteristic->setValue((uint8_t*)FLASH_BLE_START_MARKER, 4);
  flashDataCharacteristic->notify();
  delay(20);

  uint8_t totalBuf[4];
  memcpy(totalBuf, &total_bytes, 4);  // little-endian on ESP32
  flashDataCharacteristic->setValue(totalBuf, 4);
  flashDataCharacteristic->notify();
  delay(20);

  // --- Stream 243-byte chunks ---
  static uint8_t packet[FLASH_BLE_PKT_SIZE];  // static avoids stack overflow
  uint32_t read_offset = FLASH_HEADER_SIZE;
  uint32_t bytes_remaining = total_bytes;
  uint32_t chunk_index = 0;

  while (bytes_remaining > 0 && wasConnected) {
    uint16_t chunk_size = (bytes_remaining >= FLASH_BLE_PKT_SIZE)
                          ? FLASH_BLE_PKT_SIZE
                          : (uint16_t)bytes_remaining;

    esp_err_t err = esp_partition_read(dataPartition, read_offset, packet, chunk_size);
    if (err != ESP_OK) {
      Serial.printf("FLASH-BLE: Read error %d at offset %lu\n", err, read_offset);
      break;
    }

    flashDataCharacteristic->setValue(packet, chunk_size);
    flashDataCharacteristic->notify();
    ...
    // Log progress every 50 packets (~12 kB)
    if (chunk_index % 50 == 0) {
      Serial.printf("FLASH-BLE: Sent %lu chunks (%lu bytes)\n",
                    chunk_index, (uint32_t)(total_bytes - bytes_remaining));
    }

    delay(20);  // yield to BLE stack; increase to 30-50 ms if drops occur
  }
  ...
  Serial.printf("FLASH-BLE: Done — %lu chunks sent (%lu bytes total)\n",
                chunk_index, total_bytes);
  flashReadRequested = false;
}

// flashResetData()
//   Erases all recorded flash data and resets tracking variables.
//   Called when mobile sends "RESET" to the CMD characteristic.
//   After this call the device behaves as if freshly flashed.
//
void flashResetData() {
  Serial.println("FLASH-BLE: RESET — erasing all data...");
  ...
  if (dataPartition) {
    // Erase entire partition in one call (can take a few seconds for large partitions)
    unsigned long t0 = millis();
    esp_partition_erase_range(dataPartition, 0, dataPartition->size);
    Serial.printf("FLASH-BLE: Partition erased in %lu ms\n", millis() - t0);

    // Re-write fresh magic header
    uint32_t magic = FLASH_MAGIC;
    esp_partition_write(dataPartition, 0, &magic, 4);
  }
  ...
  Serial.println("FLASH-BLE: RESET complete — device ready for new recording");
  flashResetRequested = false;
}
// =====================================================================
// END FLASH-OVER-BLE FUNCTIONS
// =====================================================================
...
void setup()
{
  ...
  // 5. Flash Playback Service
  //    DATA char: mobile subscribes and receives 243-byte notify chunks
  //    CMD  char: mobile writes "READ" to start stream, "RESET" to erase
  ...
  // 6. Start advertising all three services
...
void loop() 
{
  ...
  // ===== FLASH-OVER-BLE: service mobile READ / RESET requests =====
  // These are set by FlashCmdCharacteristicCallbacks and consumed here
  // (in loop() context) so flash reads happen outside the BLE callback.
  if (flashReadRequested && wasConnected) {
    flashSendOverBLE();   // streams all stored data in 243-byte BLE packets
  }
  if (flashResetRequested) {
    flashResetData();     // erases flash and resets all tracking variables
  }
  // ===== END FLASH-OVER-BLE =====
